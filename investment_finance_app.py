...
st.set_page_config(page_title="Investment Analysis App v2", page_icon="📈", layout="wide")
...
rf_rate = st.sidebar.slider("Risk-free rate (%)", 0.0, 10.0, 4.0, 0.1) / 100
market_premium = st.sidebar.slider("Market risk premium (%)", 1.0, 12.0, 5.5, 0.1) / 100
growth_rate = st.sidebar.slider("Dividend growth rate (%)", 0.0, 12.0, 4.0, 0.1) / 100
target_pe = st.sidebar.slider("Target P/E", 5.0, 50.0, 18.0, 0.5)
var_conf = st.sidebar.slider("Historical VaR confidence", 0.90, 0.99, 0.95, 0.01)
...
# Main
# -----------------------------
st.markdown('<div class="main-header">Investment and Finance App v2</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="subtle">Interactive stock analysis, valuation, portfolio analytics, and a watchlist trend scanner built for Streamlit.</div>',
    unsafe_allow_html=True
)
...
price_df["MA5"] = price_df["Adj Close"].rolling(5).mean()
price_df["MA10"] = price_df["Adj Close"].rolling(10).mean()
price_df["MA20"] = price_df["Adj Close"].rolling(20).mean()
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
    ["Overview", "Technical", "Valuation", "Risk", "Portfolio", "Watchlist Scanner"]
)
...
with tab1:
    ...
    with c1:
        ...
        fig.add_trace(go.Scatter(x=price_df.index, y=price_df["MA5"], mode="lines", name="MA 5"))
        fig.add_trace(go.Scatter(x=price_df.index, y=price_df["MA10"], mode="lines", name="MA 10"))
        fig.add_trace(go.Scatter(x=price_df.index, y=price_df["MA20"], mode="lines", name="MA 20"))
        fig.update_layout(title=f"{ticker} Price and MAs", height=520, xaxis_title="Date", yaxis_title="Price")
    ...
    st.markdown('<div class="small-note">The app lets you change key assumptions in the sidebar so the valuation and risk outputs update automatically.</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    with tech_right:
        ...
        candle_fig.add_trace(go.Scatter(x=recent.index, y=recent["MA5"], mode="lines", name="MA 5"))
        candle_fig.add_trace(go.Scatter(x=recent.index, y=recent["MA10"], mode="lines", name="MA 10"))
        candle_fig.add_trace(go.Scatter(x=recent.index, y=recent["MA20"], mode="lines", name="MA 20"))
        candle_fig.update_layout(height=520, title=f"{ticker} Recent Candles and MAs", xaxis_rangeslider_visible=False)
...
with tab3:
    ...
    st.markdown('<div class="small-note">The DDM works best for stable dividend-paying companies. For non-dividend firms, the relative P/E section is usually more informative.</div>', unsafe_allow_html=True)
...
with tab4:
    ...
    st.write(
        f"Using a **{var_conf:.0%}** historical confidence level, the app estimates the daily loss threshold "
        f"at about **{abs(var_daily):.2%}**. CVaR shows the average loss on the worst tail days."
    )
...
with tab5:
    ...
    portfolio_period = st.selectbox("Portfolio history period", ["1y", "2y", "3y", "5y"], index=2)
    ...
    if not close_df.empty:
        ...
        aligned_weights = weights[:len(close_df.columns)]
        aligned_weights = normalize_weights(aligned_weights)
...
with tab6:
    ...
    for sym in watchlist:
        df = load_price_data(sym, period="6mo")
