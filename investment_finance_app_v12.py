...
st.set_page_config(page_title="Investment Analysis App v12", page_icon="📈", layout="wide")
...
def get_analysis_interval(period: str) -> str:
    period = str(period).strip().lower()
    if period in ["1mo", "3mo", "6mo"]:
        return "1d"
    if period in ["1y", "2y"]:
        return "1wk"
    if period in ["3y", "5y", "10y", "max"]:
        return "1mo"
    return "1d"


def get_recent_bar_count(period: str) -> int:
    period = str(period).strip().lower()
    mapping = {
        "1mo": 22,
        "3mo": 35,
...
def get_current_price_from_info(info: dict, price_df: pd.DataFrame):
    candidates = [
        safe_get(info, "currentPrice", np.nan),
        safe_get(info, "regularMarketPrice", np.nan),
        safe_get(info, "previousClose", np.nan),
        safe_get(info, "lastPrice", np.nan),
    ]
    for value in candidates:
        if pd.notna(value) and float(value) > 0:
            return float(value)

    if price_df is not None and not price_df.empty and "Adj Close" in price_df.columns:
        vals = price_df["Adj Close"].dropna()
        if len(vals) > 0:
...
def get_better_fundamentals(ticker: str, info: dict, current_price=np.nan) -> dict:
    ...
    # Market cap fallback from shares and price
    if pd.isna(result["market_cap"]) and not pd.isna(result["shares_outstanding"]) and not pd.isna(current_price) and current_price > 0:
        result["market_cap"] = float(result["shares_outstanding"] * current_price)
...
st.sidebar.caption("Bullish = Price > 10 MA and 5MA > 10MA > 20MA (using the selected lookback interval)")
...
st.markdown(
    '<div class="subtle">Adds timeframe-aware candles and trend logic, plus stronger fallback logic for market cap and other valuation fields.</div>',
    unsafe_allow_html=True
)
...
latest_price = get_current_price_from_info(info, price_df)
if pd.isna(latest_price):
    latest_price = latest["Adj Close"]

analysis_interval = get_analysis_interval(period)
analysis_df = load_price_data(ticker, period=period, interval=analysis_interval)
if analysis_df.empty or "Adj Close" not in analysis_df.columns:
    analysis_df = price_df.copy()
analysis_df = add_mas(analysis_df)

analysis_latest = analysis_df.iloc[-1]
fund = get_better_fundamentals(ticker, info, current_price=latest_price)
ma5 = analysis_latest["MA5"]
ma10 = analysis_latest["MA10"]
ma20 = analysis_latest["MA20"]
trend_label = technical_trend_label(analysis_latest["Adj Close"], ma5, ma10, ma20)
...
with tab1:
    ...
    st.markdown(
        '<div class="small-note">This version uses more fallback paths for Yahoo price downloads, timeframe-aware candles and trend logic, and better TTM EPS, dividend-history, and balance-sheet fallbacks for valuation inputs.</div>',
        unsafe_allow_html=True
    )
...
with tab2:
    ...
    with tech_right:
        signal_box(trend_label)
        st.write(
            "A stock is **Bullish** when price is above the 10-period moving average "
            "and the moving averages are stacked in the order **5MA > 10MA > 20MA**. "
            "It is **Bearish** only when price is below the 5, 10, and 20 moving averages and the reverse stack is true. "
            f"The trend signal here is calculated on the **{analysis_interval}** interval for the selected lookback period. "
            "Anything else is labeled **Not Trending**."
        )

        recent = analysis_df.tail(min(recent_candles, get_recent_bar_count(period)))
        ...
        candle_fig.update_layout(height=520, title=f"{ticker} Recent Candles and Selected MAs ({analysis_interval})", xaxis_rangeslider_visible=False)
