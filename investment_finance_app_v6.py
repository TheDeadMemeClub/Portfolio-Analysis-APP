...
st.set_page_config(page_title="Investment Analysis App v6", page_icon="📈", layout="wide")
...
def get_fcf_inputs(info: dict):
    ...
    debt = safe_get(info, "totalDebt", 0.0)
    cash = safe_get(info, "totalCash", 0.0)

    if pd.isna(fcf) and not pd.isna(market_cap) and not pd.isna(enterprise_value):
        fcf_yield_est = 0.04
        fcf = enterprise_value * fcf_yield_est
    ...
    return {
        ...
        "total_debt": debt if not pd.isna(debt) else 0.0,
        "total_cash": cash if not pd.isna(cash) else 0.0,
    }
...
with st.sidebar.expander("Valuation Inputs", expanded=True):
    ...
    relative_pe_weight = st.slider("Relative P/E weight", 0.0, 1.0, 0.4, 0.05)
    ddm_weight = st.slider("DDM weight", 0.0, 1.0, 0.2, 0.05)
    dcf_weight = st.slider("DCF weight", 0.0, 1.0, 0.4, 0.05)
...
st.markdown(
    '<div class="subtle">Cleaner layout, customizable moving averages, improved trend logic, shorter lookbacks, expanded valuation tools, and blended fair value outputs.</div>',
    unsafe_allow_html=True
)
...
fcf_inputs = get_fcf_inputs(info)
net_debt = fcf_inputs["total_debt"] - fcf_inputs["total_cash"]
dcf_value = dcf_share_value(
    fcf=fcf_inputs["free_cash_flow"],
    discount_rate=required_return,
    terminal_growth=terminal_growth_rate,
    shares_outstanding=fcf_inputs["shares_outstanding"],
    net_debt=net_debt,
    years=dcf_years,
...
blended_value = blended_fair_value(
    [relative_value, ddm_value, dcf_value],
    [relative_pe_weight, ddm_weight, dcf_weight]
)
...
with tab3:
    ...
    with right:
        ...
        st.write(f"Free cash flow: **{'N/A' if pd.isna(fcf_inputs['free_cash_flow']) else f'${fcf_inputs['free_cash_flow']:,.0f}'}**")
        st.write(f"Shares outstanding: **{'N/A' if pd.isna(fcf_inputs['shares_outstanding']) else f'{fcf_inputs['shares_outstanding']:,.0f}'}**")
    ...
    weights_df = pd.DataFrame({
        "Model": ["Relative P/E", "Dividend Discount Model", "Free Cash Flow / DCF"],
        "Weight": [relative_pe_weight, ddm_weight, dcf_weight]
    })
    ...
    st.markdown('<div class="small-note">The DDM is most useful for steady dividend-paying firms. The DCF section uses free cash flow, a forecast period, and a terminal growth assumption to estimate implied value per share. If Yahoo data is missing for a company, some model outputs may show N/A.</div>', unsafe_allow_html=True)
