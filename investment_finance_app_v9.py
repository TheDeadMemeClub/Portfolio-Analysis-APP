...
st.set_page_config(page_title="Investment Analysis App v9", page_icon="📈", layout="wide")
...
def get_better_fundamentals(ticker: str, info: dict) -> dict:
    ...
    # EPS fallback
    if pd.isna(result["trailing_eps"]) and not pd.isna(result["shares_outstanding"]) and result["shares_outstanding"] > 0 and not pd.isna(ni):
        result["trailing_eps"] = float(ni / result["shares_outstanding"])

    # Shares outstanding fallback from market cap / price if needed
    current_price_guess = safe_get(info, "currentPrice", np.nan)
    if pd.isna(current_price_guess):
        current_price_guess = safe_get(info, "regularMarketPrice", np.nan)
    if pd.isna(result["shares_outstanding"]) and not pd.isna(result["market_cap"]) and not pd.isna(current_price_guess) and current_price_guess > 0:
        result["shares_outstanding"] = float(result["market_cap"] / current_price_guess)

    # P/E fallback
    if pd.isna(result["current_pe"]) and not pd.isna(result["trailing_eps"]) and result["trailing_eps"] > 0 and not pd.isna(current_price_guess):
        result["current_pe"] = float(current_price_guess / result["trailing_eps"])

    # Book value fallback from P/B and price
    if pd.isna(result["book_value"]) and not pd.isna(result["price_to_book"]) and result["price_to_book"] > 0 and not pd.isna(current_price_guess):
        result["book_value"] = float(current_price_guess / result["price_to_book"])

    # Price to book fallback from book value and price
    if pd.isna(result["price_to_book"]) and not pd.isna(result["book_value"]) and result["book_value"] > 0 and not pd.isna(current_price_guess):
        result["price_to_book"] = float(current_price_guess / result["book_value"])
    ...
    # Dividend fallback from dividend yield if available
    div_yield = safe_get(info, "dividendYield", np.nan)
    if pd.isna(result["dividend_rate"]) and not pd.isna(div_yield) and not pd.isna(current_price_guess):
        result["dividend_rate"] = float(div_yield * current_price_guess)
...
st.markdown(
    '<div class="subtle">Fixes the Streamlit caching error, keeps the stronger valuation fallbacks, and improves reliability across more tickers.</div>',
    unsafe_allow_html=True
)
